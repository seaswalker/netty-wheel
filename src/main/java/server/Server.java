...
public final class Server implements LifeCycle {
	...
	public Server(int acceptors, int workers) {
		...
		if (acceptors > cores) {
			throw new IllegalArgumentException("The acceptors count should be less than cores.");
		}
		if (workers > cores) {
			throw new IllegalArgumentException("The workers count should be less than cores.");
		}
		...
		workerManager = new WorkerManager(workers, executor);
	...
	public Server bind(int port) {
		if (port < 1)
			throw new IllegalArgumentException("The param port can't be negative.");
		try {
	...
	public void start() {
		...
		for (int i = 0; i < acceptors; i++) executor.execute(new Acceptor());
		...
		public void run() {
			while (true) {
				try {
					...
					selectorManager.chooseOne().register(channel, SelectionKey.OP_READ);
					//触发ChannelActive
					HandlerContext context = new HandlerContext(handlerChain, true);
					workerManager.chooseOne().submit(new ChannelActiveEvent(context));
				} catch (IOException e) {
